...
class Doctor(User):
    ...
    def get_doctor(cls, data):
        query = "SELECT * FROM doctors JOIN users ON users.id = doctors.user_id JOIN areas ON areas.id = doctors.area_id WHERE doctors.id = %(id)s;"
    ...
    @classmethod
    def count_doctors(cls):
        query = "SELECT COUNT(doctors.id) AS total FROM doctors;"
