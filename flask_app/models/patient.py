...
class Patient(User):
    ...
    def get_by_id(cls, data):
        query = "SELECT * FROM patients JOIN users ON users.id = patients.user_id WHERE patients.id = %(id)s;"
        result = connectToMySQL(cls.data_name).query_db(query, data)
        return cls(result[0])

    @classmethod
    def get_all(cls): 
        query = "SELECT * FROM patients JOIN users ON users.id = patients.user_id;"
    ...
    ##Contador de total de terapeutas en centro medico
    @classmethod
    def count_patients(cls):
        query = "SELECT COUNT(patients.id) AS total FROM patients;"
        result = connectToMySQL(cls.data_name).query_db(query)
        print(result)
        return result

    ##Contador de sesiones que disminuye cada que pide una cita
    @classmethod
    def amount_selected(cls, data):
        query = "SELECT amount FROM patients WHERE patients.id = %(id)s"
        result = connectToMySQL(cls.data_name).query_db(query,data)
        return result
