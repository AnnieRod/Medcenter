...
from flask_app.models.session import User,Session
...
@app.route("/")
def start_app():
    session = Session.count_sessions()
    doctor = Doctor.count_doctors()
    patient = Patient.count_patients()
    area = Area.count_areas()
    return render_template("index.html", session = session, doctor = doctor, patient = patient, area = area)
...
def dash_doctor():
    ...
    return render_template("dashdoctor.html", doctor = Doctor.get_doctor(data), all_sessions = Session.sessions_with_patients(data))