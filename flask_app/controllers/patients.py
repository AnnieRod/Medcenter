...
from flask_app.models.session import Session, User
...
def dash_patient():
    ...
    amount = Patient.amount_selected(data)
    taken = Session.sessions_left(data)
    return render_template("dashpatient.html", patient = Patient.get_by_id(data), all_sessions = Session.show_appointments(data), all_areas = Session.patients_with_areas(data), amount = amount, taken = taken) ##SESSIONS NO FUNCIONA AUN
